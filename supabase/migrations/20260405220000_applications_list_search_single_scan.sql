-- Text search path used a matching_ids UNION (four separate applications scans + distinct + join).
-- Under RLS that could still hit statement_timeout (e.g. q=type substring). Single scan with OR
-- lets Postgres BitmapOr trgm / btree / hash plans and applies list filters once.
