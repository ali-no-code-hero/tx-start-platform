-- Unfiltered / non-text-search list was timing out: the SQL planner still costed the
-- `matching_ids` UNION branches even when search was inactive. Use plpgsql so the browse
-- path never references `matching_ids`, and add a composite sort index for keyset order.

create index if not exists applications_created_at_id_desc_idx
  on public.applications (created_at desc, id desc);
...
begin
  ...
  -- Browse path: no matching_ids / UNION — keeps default admin list under statement_timeout.
