...
begin
  ...
  select json_build_object(
    ...
    'avg_loan_requested', (
      select round(avg(f.loan_amount_requested), 2)
      from filtered f
      where f.loan_amount_requested is not null
    ),
    'median_loan_requested', (
      select round(percentile_cont(0.5) within group (order by f.loan_amount_requested), 2)
      from filtered f
      where f.loan_amount_requested is not null
    ),
    'avg_loan_approved', (
      select round(avg(f.loan_amount_approved), 2)
      from filtered f
      where f.loan_amount_approved is not null
    ),
    'median_loan_approved', (
      select round(percentile_cont(0.5) within group (order by f.loan_amount_approved), 2)
      from filtered f
      where f.loan_amount_approved is not null
    ),
    'sum_loan_requested', (
